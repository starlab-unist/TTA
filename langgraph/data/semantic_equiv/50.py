def shift_encode(input_string: str) -> str:
    """
    Encodes the input string by shifting each character forward by 5 positions in the alphabet.
    """
    encoded_chars = []
    for char in input_string:
        shifted_char = chr(((ord(char) + 5 - ord('a')) % 26) + ord('a'))
        encoded_chars.append(shifted_char)
    return ''.join(encoded_chars)


def shift_decode(encoded_string: str) -> str:
    decoded_chars = []
    index = 0
    while index < len(encoded_string):
        original_char = chr(((ord(encoded_string[index]) - 5 - ord('a')) % 26) + ord('a'))
        decoded_chars.append(original_char)
        index += 1
    return ''.join(decoded_chars)

# method name: encode_shift -> shift_encode
# method name: decode_shift -> shift_decode
# parameter name: s -> input_string for shift_encode, s -> encoded_string for shift_decode
# list comprehension replaced with for loop in shift_encode
# list comprehension replaced with while loop in shift_decode
# added comments for clarity