...
def parse_args():
    ...
    parser.add_argument("-up", "--user_prompt_dir", type=str, default="./data/prompts/translate-user-java.txt")
...
def translate(args):
    ...
    for batch_idx in tqdm(range(n_iter)):
        ...
        for i, source in enumerate(sources[start_idx:end_idx]):
            
            messages = [
                {"role": "system", "content": system},
                {"role": "user", "content": user.format(source_code=source, class_name=f"HumanEval_{batch_idx * args.batch_size + i}")}
            ]
        ...
        for response in responses:
            ...
            with open(output_dir + "HumanEval_{}.java".format(idx), 'w') as f:
